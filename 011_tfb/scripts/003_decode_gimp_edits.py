...
Este módulo revierte el proceso de curación visual. Toma las imágenes RGB editadas
manualmente en GIMP (donde se han corregido las inferencias de la IA) y las
...
def main():
    ...
    if not gimp_files:
        print(
            "[!] No se han encontrado archivos editados (_SCL_UNET_GIMP.tif) en la carpeta visualizations/SCL_UNET"
        )
        print(
    ...
    for gimp_file in gimp_files:
        ...
        if original_tif.exists():
            ...
            print(
                "    [!] Aviso: No se encontró el TIF matemático original. Se guardará usando la cabecera de GIMP."
            )
