"""
Script para la creación de parches de datos (tiling) a partir de gránulos Sentinel-2.

Este módulo fragmenta imágenes masivas en ventanas de 512x512 píxeles para optimizar
la memoria VRAM durante el entrenamiento de la red U-Net. Además, calcula el
índice NDSI (Normalized Difference Snow Index) para facilitar la separabilidad
espectral entre nieve y nubes.
"""
...
def load_and_resample(file_path, target_shape, is_categorical=False):
    ...
    with rasterio.open(file_path) as dataset:
        data = dataset.read(1, out_shape=target_shape, resampling=resampling_method)
    return data
