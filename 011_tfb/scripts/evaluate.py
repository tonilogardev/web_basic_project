...
def main():
    ...
    print(f"[*] Se encontraron {len(edited_files)} archivos de Test curados manualmente.")
    ...
    for gt_path in edited_files:
        ...
        if not pred_path.exists():
            print(f"    [!] Advertencia: No se encontró la predicción para {gt_path.name}")
            continue
    ...
    precision = np.divide(np.diag(cm), sum_col, out=np.zeros_like(sum_col, dtype=float), where=sum_col!=0)
    
    # Recall = TP / (TP + FN) -> diag / sum_row
    sum_row = cm.sum(axis=1)
    recall = np.divide(np.diag(cm), sum_row, out=np.zeros_like(sum_row, dtype=float), where=sum_row!=0)
    ...
    print(f"{'':>15} | {'Pred: Suelo':>12} | {'Pred: Nube':>12} | {'Pred: Sombra':>12} | {'Pred: Nieve':>12}")
    print("-" * 75)
    for i, class_name in enumerate(['Real: Suelo', 'Real: Nube', 'Real: Sombra', 'Real: Nieve']):
        row = cm[i]
        print(f"{class_name:>15} | {row[0]:>12,} | {row[1]:>12,} | {row[2]:>12,} | {row[3]:>12,}")
    ...
    plt.title('Matriz de Confusión Agregada - Test Set\n(Verdad Terreno vs Predicción U-Net)')
