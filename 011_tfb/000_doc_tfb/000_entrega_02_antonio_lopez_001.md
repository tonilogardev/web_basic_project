...
### Evolución de la Taxonomía: Transición al modelo de 6 clases
La arquitectura inicial del proyecto contemplaba un ecosistema de cinco clases. Sin embargo, tras la ejecución de una primera inferencia y la evaluación de su matriz de confusión, se comprobó empíricamente una deficiencia estructural: el modelo era matemáticamente incapaz de aislar las masas de agua (océanos y arrozales inundados), confundiéndolas recurrentemente con las clases "Sombra" y "NoData" debido a la extrema absorción de la banda infrarroja (SWIR). Ante esta insuficiencia del enfoque inicial, se tomó la decisión estratégica de detener la línea de producción y redefinir la taxonomía a un modelo robusto de seis clases (incluyendo explícitamente "Masas de Agua"). Para lograrlo, fue imperativo intervenir manualmente la base de datos (Verdad Terreno) para curar el sesgo heredado, lo que motivó el desarrollo de la arquitectura de edición visual descrita a continuación.
...
El proceso se inicia sin intervención manual. Durante la descarga de nuevos gránulos ([`002_download_test.py`](../scripts/002_download_test.py)) o la ejecución de inferencia ([`006_predict.py`](../scripts/006_predict.py)), el sistema genera automáticamente archivos de sufijo `_GIMP.tif`.
*Ejemplo:* `visualizations/SCL_UNET/TE_01_SCL_UNET_GIMP.tif`

##### Paso B: Edición Fotográfica
1. Se abre el archivo `_GIMP.tif` en el software de edición gráfica.
2. Se cargan las imágenes ópticas de referencia (`ColorReal.tif` o `FalsoColor_Nieve.tif`) como capas subyacentes para validar la realidad física del terreno.
3. Mediante la herramienta de selección de color (cuentagotas), se captura el color oficial de la clase a aplicar.
4. Se emplea el lápiz digital (sin interpolación geométrica o *antialiasing*) para corregir los píxeles anómalos.
5. Se sobrescribe el archivo resultante.

##### Paso C: Decodificación y Recuperación
Finalmente, desde el entorno de ejecución, se invoca el decodificador:
...
Este procedimiento detecta los archivos manipulados y genera las versiones matemáticas definitivas `[ID]_SCL_edited.tif`, las cuales constituyen la **Verdad Terreno** final validada.
...
Para subsanar el muro analítico detectado en las discusiones respecto a las "sombras orográficas", se plantea como línea futura la integración de **Modelos Digitales de Elevaciones (DEM)** proporcionados por el Instituto Cartográfico de Cataluña (ICGC). Es imperativo aclarar que el uso de topografía no se utilizará para discriminar entre nieve y nubes (reto ya superado mediante las bandas espectrales), sino de forma exclusiva para solucionar los fallos en la detección de sombras nubosas frente a las sombras proyectadas por el relieve montañoso. Esta inyección altimétrica brindará consciencia tridimensional a la red neuronal, erradicando la confusión geométrica de las sombras.
...
Finalmente, los pesos lógicos (*checkpoints*) de la U-Net entrenada a máximo rendimiento deberán desacoplarse del ecosistema de entrenamiento local. La meta culminante para la entrega final contempla el desarrollo y despliegue de una Aplicación Web interactiva (*Serverless*) basada en estándares *Cloud Optimized GeoTIFF* y *PMTiles*. Para garantizar la viabilidad computacional y económica de este entorno, se rediseñará la canalización de datos geoespaciales: mientras que la inferencia del modelo se mantendrá en PyTorch (Python), las piezas críticas de ingesta masiva de tensores y procesamiento de metadatos se implementarán en **Rust**. Este lenguaje de sistemas permitirá optimizar drásticamente el uso de memoria y la velocidad de ejecución, evitando los cuellos de botella inherentes a Python en *pipelines* de alto rendimiento. Esta plataforma conectará directamente con el flujo de datos de Copernicus para que, cada vez que el satélite Sentinel-2 registre una nueva imagen sobre la región de Cataluña, la aplicación web ejecute la Inteligencia Artificial de forma automática. Esto proveerá al usuario final de máscaras de nubes depuradas casi en tiempo real, operables desde un simple navegador web, clausurando de forma exitosa el perímetro trazado en los objetivos fundacionales. Cabe destacar que la concepción de esta infraestructura supondrá un **reto tecnológico y de conocimiento formidable a nivel personal**, dado que trasladar un modelo de Inteligencia Artificial desde un entorno aislado de investigación hacia un servidor web en producción, garantizando su estabilidad y conectividad, es un terreno inexplorado para el autor y marcará el hito formativo más ambicioso de este Trabajo Final.
