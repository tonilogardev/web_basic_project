...
<h2 align="center" style="color: #FFC000; font-size: 1.8em; margin-bottom: 40px;">Desarrollar un entorno escalable (pipeline geoespacial) para imágenes Sentinel-2, diseñado para integrar y ejecutar un modelo de Machine Learning</h2>
...
<div align="center" style="font-size: 1.4em; line-height: 1.6; margin-bottom: 40px;">
  ...
  <strong>Fase:</strong> Entrega 3: ENTREGA COMPLETA DEL TFT<br>
...
- **Reducción de dimensionalidad de clases:** El estándar europeo divide el terreno en 12 categorías, aportando ruido computacional e ineficiencia. Como pilar metodológico, el *pipeline* geoespacial desarrollado colapsa matemáticamente esas 12 clases originales en **[6 Clases Maestras](008_pixel_legend.md)**: No data, nube, sombra, nieve, vegetación/suelo, agua.  
...
Para el rigor académico y técnico de este Trabajo Final de Bàtxelor se enmarca dentro de las clasificaciones científicas internacionales y persigue un impacto directo en la sostenibilidad global.
...
- **Bandas Ópticas y de Infrarrojo Cercano a 10m de resolución (`B02.jp2`, `B03.jp2`, `B04.jp2`, `B08.jp2`):** Estos cuatro ficheros constituyen la piedra angular óptica del proyecto. Representan, respectivamente, las longitudes de onda del espectro visible (Azul, Verde y Rojo) y el espectro del Infrarrojo Cercano (NIR). En la fase de entrenamiento de nuestra Inteligencia Artificial, estos archivos `.jp2` son leídos en memoria y concatenados matemáticamente para conformar las primeras cuatro dimensiones espaciales de nuestro tensor de entrada multicanal (denominado formalmente $X$). Mantener su estado puro original L1C (reflectancia TOA o *Top of Atmosphere*) evita que el modelo herede los sesgos algorítmicos introducidos por los módulos de corrección atmosférica de la ESA.

- **Bandas de Infrarrojo de Onda Corta o SWIR a 20m de resolución (`B11.jp2`, `B12.jp2`):** El espectro SWIR posee la capacidad física de penetrar brumas finas e interactuar térmicamente con las moléculas de agua. Esto resulta indispensable para que nuestra red neuronal U-Net logre diferenciar matemáticamente una nube de gran grosor (compuesta por vapor de agua) del hielo o la nieve (agua en estado sólido), los cuales presentan una reflectividad casi idéntica en el espectro visible, originando los habituales falsos positivos en los sistemas de teledetección tradicionales.

- **Entornos Virtuales de Composición GDAL (`ColorReal.vrt` y `FalsoColor_Nieve.vrt`):** Lejos de generar enormes archivos fusionados que colapsarían la capacidad de almacenamiento, el *pipeline* implementa la estrategia de los ficheros VRT (*Virtual Raster Dataset*). Se trata de estructuras XML extremadamente ligeras (de unos pocos kilobytes) que actúan como "punteros" inteligentes. Enlazan en tiempo real las distintas bandas individuales `.jp2` almacenadas en la misma carpeta para crear composiciones multicanal al vuelo (Rojo-Verde-Azul para visualización humana, y combinaciones que incluyen SWIR para resaltar nieve o calcinaciones). Esta optimización arquitectónica permite a los analistas abrir en QGIS las escenas completas de más de 100 kilómetros cuadrados de forma instantánea, sin que se generen ficheros redundantes masivos en el disco duro.

- **Miniatura Rápida de Catalogación Visual (`preview.png`):** Generada de forma cien por cien autónoma por el sistema, esta imagen estándar condensa la vastedad de los datos satelitales en un archivo microscópico. Su rol es puramente operativo: permite a los investigadores realizar un descarte visual rápido desde el explorador de archivos del sistema operativo o desde una interfaz web, evitando así el ingente coste computacional en ciclos de CPU y RAM que supondría tener que abrir y renderizar las pesadas matrices de datos `.jp2` para comprobar empíricamente si la escena está, por ejemplo, totalmente oscurecida por una tormenta profunda y carece de valor analítico.

- **Lienzo Base de Clasificación Oficial (`SCL.tif`):** Este archivo es la semilla sobre la cual se cimienta el trabajo manual del proyecto. Contiene el mapa de píxeles (la máscara) clasificado por el algoritmo algorítmico tradicional de la ESA, Sen2Cor. El *pipeline* no solo extrae este mapa de la escena L2A, sino que en tiempo de ejecución colapsa sus 12 categorías nativas e ineficientes hacia nuestro estándar de 6 Clases, transformándo el formato `.jp2` al estándar abierto y universal *Cloud Optimized GeoTIFF* (*COG*).
...
- **Componentes del Ecosistema del Puente GIMP (`SCL_GIMP.tif` y `SCL_GIMP_temp.tfw`):** Constituyen la innovación más ingeniosa para democratizar la limpieza de ruido geográfico. Puesto que las herramientas avanzadas de retoque de código abierto como GIMP no están concebidas para el formato geoespacial y destruyen las coordenadas del sistema de proyección (EPSG) al guardar, este par de archivos orquesta la compatibilidad. El fichero `.tif` es una versión temporal a la que se le inyecta programáticamente una paleta de colores RGBA (Rojo, Verde, Azul, Alfa) para que pueda ser pintada con un pincel convencional de un editor fotográfico. Simultáneamente, el *World File* (`.tfw`), un archivo de texto minúsculo con 6 líneas matemáticas, ancla el tamaño exacto del píxel geográfico y las coordenadas absolutas de las esquinas del raster. Una vez concluida la edición artística, el script de Python lee el `.tfw` para restaurar geográficamente la imagen, devolviéndola al mundo de la teledetección convertida en nuestra inmaculada Verdad Terreno definitiva ($Y_{true}$).
...
<div align="center" style="margin: 20px 0;">
  <img src="img/data_download_and_gimp_files.png" alt="Estructura de archivos L1C y SCL generada por los scripts" width="80%">
  <br><em>Figura: Estructura de archivos resultante tras la orquestación de descargas automatizadas.</em>
</div>
...
<div align="center" style="margin: 20px 0;">
  <img src="img/edit_clas_gimp.png" alt="Proceso de edición manual de píxeles en GIMP" width="90%">
  <br><em>Figura: Proceso de clasificación manual y purificación de píxeles utilizando el editor GIMP.</em>
</div>


### 8.2.3. Gestión y almacenamiento

Una vez que poseemos las imagenes brutas L1C y nuestra Verdad Terreno editada y clasificada manualmente, los datos satelitales (gránulos enteros de 100x100 kilómetros) son aún demasiado masivos  para ser procesados por una tarjeta gráfica. Para resolver este cuello de botella, entra en juego el *script* de ingeniería de datos [`004_create_dataset.py`](../scripts/004_create_dataset.py). 
...
Con los tensores serializados listos, la fase de modelado cobra vida a través del *script* [`005_train.py`](../scripts/005_train.py). Este módulo es el núcleo del aprendizaje profundo (*Deep Learning*) del proyecto, donde se construye e instruye a la arquitectura U-Net nativa desde cero utilizando PyTorch.

El ciclo de entrenamiento se rige por las siguientes características arquitectónicas:
1. **Topología Convolucional:** La red neuronal U-Net se ha programado para aceptar una primera capa de entrada (*Input Layer*) de 7 canales (las 6 bandas ópticas y térmicas + NDSI) y expulsar un mapa de segmentación bidimensional de 6 canales (las probabilidades de pertenencia a cada una de nuestras 6 Clases Maestras).
...
Habiendo validado matemáticamente la robustez de la red neuronal frente al algoritmo estándar, el proyecto culmina poniendo en valor el modelo entrenado. Para que la inferencia tenga una aplicación técnica real y escalable, se ha desarrollado el *script* final [`009_cloud_model_catalonia_pmtiles.py`](../scripts/009_cloud_model_catalonia_pmtiles.py).
...
3. **Formateo Web-Native:** Para habilitar un consumo ultra-rápido en la web, la máscara matemática cruda es coloreada dinámicamente mediante GDAL (asignando píxeles blancos semitransparentes a las nubes) y transformada junto con la banda óptica a formato **PMTiles** (Cloud-Native Raster Tiles).
4. **Auto-limpieza (*Garbage Collection*):** Inmediatamente después de extraer el valor analítico (los archivos `.pmtiles`), el *script* destruye de forma automática los enormes ficheros crudos originales del satélite para evitar el colapso del disco duro del servidor.

De manera permanente, el sistema retiene en el disco físico (dentro de la ruta `visualizations/SCL_UNET_catalonia/`) una colección estructurada de productos analíticos derivados de la inferencia. La siguiente figura ilustra el abanico de archivos generados por cada gránulo procesado, los cuales cumplen funciones dispares, desde la validación científica hasta la exposición interactiva en la web:

<div align="center" style="margin: 20px 0;">
  <img src="img/files_script_009_cloud_model_catalonia_pmtiles.png" alt="Estructura de archivos resultantes tras la inferencia Serverless" width="80%">
  <br><em>Figura 4: Estructura de archivos generados en producción tras la inferencia de la red neuronal.</em>
</div>

**Productos Ópticos de Referencia (Color Real y Falso Color):**
- **`*_ColorReal.tif` y `*_FalsoColor_Nieve.tif`:** GeoTIFFs de alta resolución que contienen las composiciones multiespectrales ensambladas a partir de las reflectancias L1C crudas. El primero representa el espectro visible humano (RGB), mientras que el segundo integra las bandas SWIR para destacar el hielo y la nieve en tonalidades cian brillantes. Son fundamentales para la validación forense y el contraste visual humano.
- **Archivos de georreferenciación (`.tfw`):** Pequeños ficheros *World File* de texto plano que anclan matemáticamente los píxeles de las imágenes ópticas anteriores a sus coordenadas cartográficas absolutas.

**Productos Analíticos de la Red Neuronal (Clasificación):**
- **`*_SCL_UNET.tif`:** La predicción matemática pura y cruda expulsada por la red U-Net. Es un raster de una sola banda (8-bits) que contiene las 6 Clases Maestras (valores numéricos discretos) inferidas píxel a píxel por la inteligencia artificial.
- **`*_SCL_UNET_GIMP.tif`:** Una variante del raster matemático a la que se inyecta programáticamente una paleta de colores estandarizada. Está diseñada para facilitar la auditoría visual por parte de un analista humano sin necesidad de aplicar estilos manuales en el software GIS.
- **`*_SCL_UNET_mask_clouds.tif`:** Una máscara binaria derivada algorítmicamente que agrupa las clases en dos estados puros: "Nubes y Sombras" frente a "Terreno Despejado" (suelo, vegetación y nieve). Matemáticamente, asigna un valor lógico positivo (que visualmente se traduce a blanco) a todo lo que *no* es nube, dejando las nubes y sus sombras como valor nulo (negro). Constituye el paso previo y esencial a la publicación web.
...
El eslabón final de la arquitectura del proyecto ha consistido en el desarrollo íntegro de una aplicación interactiva (Frontend) que permite a cualquier usuario consumir y superponer las predicciones del modelo U-Net sobre cartografía base.

Esta plataforma web interactiva se ha construido sobre el *framework* reactivo **Svelte** empaquetado con Vite, y utiliza la librería **MapLibre GL JS** nativamente. Mediante la integración del protocolo personalizado `pmtiles://`, el cliente web es capaz de decodificar directamente los ficheros físicos `.pmtiles` estáticos sin necesidad de un servidor geoespacial pesado como GeoServer.

El visor dispone de un diseño *Dark Mode* que maximiza el contraste de las nubes blancas e incorpora una barra lateral reactiva. Mediante los enlaces de estado dinámico de Svelte, el usuario final cuenta con controles para:
- Conmutar instantáneamente la visibilidad de la imagen de satélite original y de la capa de predicción generada por la IA.
- Controlar fluidamente la opacidad de la máscara de nubes mediante un deslizador interpolado para facilitar la auditoría manual de los bordes de la predicción frente a la imagen óptica subyacente.

Todo el frontal ha sido contenerizado mediante Docker e integrado en el ecosistema de orquestación de la plataforma mediante Traefik, quedando expuesto y accesible públicamente desde la *Homepage* del portfolio.

<div align="center" style="margin: 20px 0;">
  <img src="img/Screenshot_2026-08-29_20-18-02.png" alt="Interfaz interactiva del Visor Web GIS" width="100%">
  <br><em>Figura 5: Interfaz interactiva del Visor Cartográfico (Frontend Svelte + MapLibre GL JS) exponiendo las predicciones del modelo en producción.</em>
</div>  
...
5. **Mejoras Evolutivas del Visor Cartográfico (Web GIS):**
   Para transformar el prototipo web actual en una herramienta de investigación de grado de producción, la plataforma debe incorporar nuevas funcionalidades centradas en el consumo avanzado de datos:
   - **Sistema de Descarga Geoespacial:** Habilitar un módulo de exportación asíncrono que permita al usuario dibujar un polígono de interés (Bounding Box) en el mapa y descargar empaquetados los datos crudos (`.tif` L1C) y sus máscaras binarias asociadas para consumirlos en su SIG local.
   - **Análisis Estadístico *On-the-fly*:** Implementar herramientas de análisis espacial en el cliente (usando librerías como Turf.js) que calculen al vuelo el porcentaje total de nubosidad de una comarca seleccionada, o generen gráficos de series temporales de la cobertura nival.
   - **Mecanismo de *Crowdsourcing* (Corrección Humana):** Un sistema interactivo donde meteorólogos o investigadores puedan corregir visualmente en el mapa los falsos positivos de la Inteligencia Artificial. Este *feedback* se enviaría de vuelta al *backend* para alimentar y reentrenar iterativamente la red neuronal, cerrando el ciclo de *Active Learning*.

6. **Evolución de la Arquitectura Deep Learning:**  
Superar las limitaciones del modelo base futuras.

- **Integración de Bloques Residuales (ResUNet):** Sustituir las convoluciones estándar del Encoder por bloques residuales. Esta modificación mitigaría el problema del desvanecimiento del gradiente (vanishing gradient), permitiendo el entrenamiento de redes topológicamente más profundas con mayor capacidad de extracción de características abstractas.  

- **Mecanismos de Atención (Attention U-Net):** La introducción de puertas de atención (Attention Gates) en las Skip Connections permitiría al modelo suprimir matemáticamente la información redundante del fondo de la imagen. Esto focalizaría la capacidad de inferencia en las áreas de alta ambigüedad espectral, mejorando la discriminación en los límites difusos entre la nieve de alta montaña y la nubosidad gruesa.  
